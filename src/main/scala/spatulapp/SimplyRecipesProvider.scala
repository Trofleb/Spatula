...
object SimplyRecipesProvider extends RecipeProvider {
  ...
  def parseRecipe(originUrl: String)(html: String): Option[Recipe] = {
		...
		val postId = jQuery($.find("#content > article:first")).prop("id").toString
		val picture = Some(jQuery($.find(s"#$postId > div.featured-image > img:first")).prop("src").toString)
		val title = jQuery($.find(s"#$postId > header > h1")).html().toString
		val stars = scala.util.Random.nextInt(6)
		val pp = jQuery($.find(s"#$postId > div.entry-content > div.recipe-callout > div.entry-details.recipe-ingredients > ul > li"))
		val ingredients = pp.toArray().map (x => jQuery(x).html().toString)
		val tt = jQuery($.find(s"#$postId > div.entry-content > div.recipe-callout > div.entry-details.recipe-method.instructions > div > p"))
  ...
  def parseResults(html: String): Future[Seq[Recipe]] = {
  	val $ = jQuery(jQuery.parseHTML(html))
  	val entryList = $.find("#content > div.entry-list > ul")
		val hrefs = entryList.find("li > [href]").toArray()
